...
This index is complete for the public `ellmos-ai` repositories at the time it was last fully audited, plus incremental additions since. Live count (verified via `gh repo list`, 2026-08-18): 62 active public repos, 1 archived (63 total). This file was updated to add `grounding-seed` and `source-resolver` (published 2026-08-18). Known gap, not addressed in this pass: `pasta-press` and `sentinel-fleet` are also public but not yet listed here — due for a separate maintenance pass. Last checked against GitHub: 2026-08-18.
